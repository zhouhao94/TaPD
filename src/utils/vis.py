...
def _plot_static_map_elements(
    ...
    for lane_segment in static_map.vector_lane_segments.values():
        ...
        _plot_polylines(
            [centerline], line_width=1.0, color="#000000", alpha=0.2, style="--"
        )
        _plot_polylines(
            [centerline],
            line_width=1,
...
def _plot_actor_tracks(
    ...
    for track in scenario.tracks:
        ...
        history_trajectory_10: NDArrayFloat = np.array(
            [
                list(object_state.position)
                for object_state in track.object_states
                if object_state.timestep <= timestep and object_state.timestep >= 40
            ]
        )
        history_trajectory_30: NDArrayFloat = np.array(
            [
                list(object_state.position)
                for object_state in track.object_states
                if object_state.timestep <= timestep and object_state.timestep >= 20
            ]
        )
        history_trajectory_50: NDArrayFloat = np.array(
            [
                list(object_state.position)
                for object_state in track.object_states
                if object_state.timestep <= timestep
        ...
        track_color = _DEFAULT_ACTOR_COLOR
        if track.track_id == 'AV' or track.track_id == '11289' or track.track_id == '11307':
            continue
        # _scatter_polylines([history_trajectory], color="red", linewidth=2, arrow=False, grad_color=False)
        _scatter_polylines([history_trajectory_50], color="yellow", linewidth=4, arrow=False, grad_color=False)
        _scatter_polylines([history_trajectory_30], color="red", linewidth=5, arrow=False, grad_color=False)
        _scatter_polylines([history_trajectory_10], color="black", linewidth=6, arrow=False, grad_color=False)
        if track.track_id == focal_id:
            track_bounds = history_trajectory_50[-1]
        ...
        # else:
        #     plt.plot(
        #         history_trajectory_50[-1, 0],
        #         history_trajectory_50[-1, 1],
        #         "o",
        #         color=track_color,
        #         markersize=2,
        #     )
