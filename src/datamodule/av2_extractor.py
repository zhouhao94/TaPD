...
class Av2Extractor:
    ...
    def process(self, raw_path: str, agent_id=None):
        ...
        for actor_id, actor_df in df.groupby("track_id"):
            ...
            pos_xy_np = np.stack(
                [
                    actor_df["position_x"].to_numpy(dtype=np.float32, copy=True),
                    actor_df["position_y"].to_numpy(dtype=np.float32, copy=True),
                ],
                axis=-1,
            )
            pos_xy = torch.as_tensor(pos_xy_np, dtype=torch.float32)

            heading_np = actor_df["heading"].to_numpy(dtype=np.float32, copy=True)
            heading = torch.as_tensor(heading_np, dtype=torch.float32)
            velocity_np = actor_df[["velocity_x", "velocity_y"]].to_numpy(
                dtype=np.float32,
                copy=True,
            )
            velocity = torch.as_tensor(velocity_np, dtype=torch.float32)
    ...
    def get_lane_features(
        ...
        for segment in lane_segments:
            ...
            lane_centerline_np = np.asarray(lane_centerline[:, :2], dtype=np.float32)
            lane_centerline = torch.as_tensor(lane_centerline_np, dtype=torch.float32)
